...
void Cms_1303_2985_CR::initialize() {
  ...
  void Cms_1303_2985_CR::analyze() {
    ...
    int nJets = jets.size();    
    photons = filterPhaseSpace(photons, 25., -2.5, 2.5);
    ...
    jets = overlapRemoval(jets, photons, 0.3);
    nJets = jets.size();
    ...
    if ((nPhotons > 0) && (photons[0]->PT > 165.0) && (fabs(photons[0]->Eta) < 1.45)) {
      for (int j = 0; j < nJetsEta; j++) {
        if (photons[0]->P4().DeltaR(jetsEta[j]->P4()) < 1.)
          return;
      }
      crY = true;
    }
