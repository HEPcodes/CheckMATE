...
void Atlas_conf_2013_047_CR::analyze() {
  ...
  photons = filterPhaseSpace(photons, 130., -2.47, 2.47);
  std::vector<Photon*> tempPhotons;    
  for (int i = 0; i < photons.size(); i++) {
    Photon* cand = photons[i];
  ...
  if (nPhotons == 1) {
    cry = true;
    double newMissPx = photons[0]->P4().Px() + missingET->P4().Px();
    double newMissPy = photons[0]->P4().Py() + missingET->P4().Py();
