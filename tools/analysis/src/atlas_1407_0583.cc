...
std::string Atlas_1407_0583::tNbC_mix_Num[13] = {"00_jet1", "01_jet2", "02_jet3", "03_bTag", "04_bJetPT", "05_dPhiJet1", "06_dPhiJet2", "07_dPhiLepETMiss", "08_dRLepJet1", "09_dRLepJet2", "10_EtMiss", "11_met/ht", "12_mT"};
...
void Atlas_1407_0583::analyze() {
  ...
  if( (nBTags70 > 0) && (tN_high == true) ){
    ...
    /// Put for loop in for either option but using hardest jet to be conservative
    double dRMin =  10.;
    TLorentzVector bJet;
    //for(int i = 0; i < jetsNorm.size(); i++){
    for(int i = 0; i < 1; i++){  
	...
	for(int i = 0; i < jetsNorm.size(); i++){
	  if(jetsNorm[i]->PT <100.){
	    sigma2 += pow( (jetsNorm[i]->PT*(((jetsNorm[i]->PT-20)*(-0.05/80))+0.10)) ,2);
	    missHT -= jetsNorm[i]->P4();
	  }
	  else{
